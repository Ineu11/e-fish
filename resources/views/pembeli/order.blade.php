...
@if (session('error'))
<div id="notif"
  class="fixed top-2 right-5 bg-red-600 text-white px-8 py-3 rounded-lg shadow-lg opacity-0 transition duration-700" style="margin-top: 5%" style="padding-bottom: 10%">
  {{ session('error') }}
</div>
...
@endif
<div class="max-w-3xl mx-auto bg-white shadow-lg rounded-xl p-6 border border-gray-200">
...
<a href="{{ route('welcome') }}"
  ...
  <form action="{{ route('order.store') }}" method="POST" class="space-y-4">
    ...
    <div>
      <label class="block text-sm font-medium mb-1">Jumlah Pesanan</label>
